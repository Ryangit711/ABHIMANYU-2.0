...
| **GitHub Mobile Proofreading (REFINEMENT — Instant .md Push on LIFTOFF/SHOOT)** | During LIFTOFF and SHOOT: after writing ALL .md files (SHOOT, INTEL, NETWORKING, SUBMISSION_BLUEPRINT) to both Linux dir AND OneDrive, push to Git IMMEDIATELY — before generating DOCX. This lets Aman proofread the full package on the GitHub mobile app while on the go. The .md files are the review layer; DOCX is the deploy layer. Separate the two: push .md first for mobile review → wait for YES → generate DOCX. This refinement does NOT override "Push after EVERY prompt" — it adds a MID-CYCLE push so the thinker can review from anywhere. |
